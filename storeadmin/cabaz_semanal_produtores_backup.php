...
$(function() {
	...
	$('#CabazProdutores').dataTable( {
		...
		"aaSorting": [[ 3, "desc" ]],
			//"bInfo": false,
	        "sAjaxSource": "../ajax/aj_getCabazProdutores_backup.php",
    	...
    	if ( anSelected.length == 1 ) {
    		var id_encomendaProdutor = anSelected.find("td").first().html();	    	
		var name = anSelected.find("td").first().next().html();
		var total = anSelected.find("td").first().next().next().html();
		var id_produtor = anSelected.find("td").first().next().next().next().next().html();
			
			self.location='ver_encomenda_produtor_backup.php?pid=' + id_encomendaProdutor + '&n=' + name+'&t='+total+ '&idp='+id_produtor;
		}
	});	
});
</script>
