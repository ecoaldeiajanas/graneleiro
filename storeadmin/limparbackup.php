...
if(isset($_GET['new'])){
	$ctr=1;
	$msg="Registos removidos com sucesso!";
}
?>

<?php
//Delete item question to admin and delete ENCOMENDAS if they choose
if(isset($_GET['yesdelete'])){
	

	$q = $conDB->sql_query("DELETE FROM encomenda WHERE semana=1", BEGIN_TRANSACTION);	

	$q = $conDB->sql_query("DELETE FROM encomendaprodutor_has_products", BEGIN_TRANSACTION);
	
	$q = $conDB->sql_query("DELETE FROM encomendaprodutor", BEGIN_TRANSACTION);
	
	$q = $conDB->sql_query("",END_TRANSACTION);

	header("location:../storeadmin/limparbackup.php?new=1");
		exit();
}
?>
...
<div id="motherContainer">
    ...
    <div id="pageContainer">
		...
		<li><a href="cabaz_semanal_consumidores_backup.php">CONSUMIDORES</a></li>
                ...
                <li style="padding: 15px 0;padding-left: 10px;">REMOVER REGISTOS</li>
          ...
          <h2>Remover Registos</h2><hr>
          		<div align="left" style="margin-left:10px ">
			
                 	<?php 	
			if($ctr==0){ 
			echo 'Deseja remover os registos das encomendas?<a href="limparbackup.php?yesdelete"> Sim </a> | <a href="../storeadmin/limparbackup.php">Não</a>';
			}else{ 
         		echo $msg;
			}
			?>
